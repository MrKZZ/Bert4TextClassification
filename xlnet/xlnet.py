...
'''
@File    :   xlnet.py
@Time    :   2019/11/20 22:24:20
@Author  :   Kang Zhezhou
@Version :   1.0
@Contact :   kangzhezhou18@mails.ucas.ac.cn, kangzhezhou@iie.ac.cn
@License :   (C)Copyright 2014-3XXX, kang NLP
@Description : 
    xlnet 使用 transformers 实现文本分类任务
'''

import torch
from transformers import InputExample, XLNetTokenizer, WEIGHTS_NAME, WarmupLinearSchedule, AdamW, XLNetForSequenceClassification, XLNetConfig
...
from Xlnet_eval import evaluate
...
if __name__ == "__main__":
    ...
    config = config_class.from_pretrained(pretrained_weights, num_labels=10)
    tokenizer = tokenizer_class.from_pretrained(pretrained_weights)
    if args.retrain:
        model = model_class.from_pretrained(args.output_dir+"/checkpoint-final")
    else:
        model = model_class.from_pretrained(pretrained_weights, config=config)#, num_labels=10)
